...
const logInViaGoogle = async (
  ...
  const userNameList = (user?.names && user.names) || null;
  const userPhotoList = (user?.photos && user.photos) || null;
  const userEmailsList = (user?.emailAddresses && user.emailAddresses) || null;
  ...
  Query: {
    authUrl: (): string => {
      try {
        ...
        throw new Error(`Failed to query Google Url.${error}`);
      }
    },
  },
  Mutation: {
